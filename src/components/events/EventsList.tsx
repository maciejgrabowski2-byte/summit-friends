      ...
      {Object.entries(eventsData).map(([dateGroup, events]) => (
        <div key={dateGroup}>
          ...
          <div className="hidden sm:flex items-center gap-4 mb-4 px-4">
            ...
            <span className="w-32 shrink-0 text-sm text-muted-foreground">Departing from</span>
            <span className="w-48 shrink-0 text-sm text-muted-foreground">Activity</span>
            <span className="w-40 shrink-0 text-right text-sm text-muted-foreground">Participants</span>
          </div>
          <h2 className="sm:hidden text-lg font-semibold text-foreground mb-4">{dateGroup}</h2>
